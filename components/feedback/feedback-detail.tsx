...
import { getFeedbackById, updateFeedback, respondToFeedback, deleteFeedback } from "@/lib/firebase-service"
      ...
      <div className="space-y-6 md:ml-64">
        ...
        <Card>
          <CardContent className="pt-6">
            <div className="text-center py-8 text-muted-foreground">Feedback not found</div>
          </CardContent>
        </Card>
      </div>
    )
        ...
        <AlertDialog>
          ...
          <AlertDialogContent>
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                This action cannot be undone. This will permanently delete this feedback from the database.
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
        ...
        <CardHeader>
          <div className="flex justify-between items-start">
            <div>
              ...
              <CardDescription>View and respond to this feedback</CardDescription>
            </div>
            <Badge
              variant={
                feedback.status === "pending" ? "outline" : feedback.status === "responded" ? "default" : "secondary"
              }
            >
              {feedback.status}
            </Badge>
        ...
        <CardContent className="space-y-6">
          <div className="grid gap-6 md:grid-cols-2">
            <div>
              ...
              <div className="space-y-1">
                ...
                <p className="text-sm">
                  <span className="font-medium">Submitted:</span> {formatDate(feedback.createdAt)}
                </p>
              </div>
            </div>
            <div>
          ...
          {feedback.response && (
            <div>
              ...
              <div className="p-3 bg-muted rounded-md">
                <p className="text-sm whitespace-pre-line">{feedback.response}</p>
                {feedback.responseDate && (
                  <p className="text-xs text-muted-foreground mt-2">Responded on {formatDate(feedback.responseDate)}</p>
                )}
              </div>
            </div>
          )}
          ...
          <div className="space-y-2">
            ...
            <div className="flex gap-2">
              <Select value={status} onValueChange={(value: "pending" | "reviewed" | "responded") => setStatus(value)}>
              ...
              <Button onClick={handleStatusChange} disabled={submitting || status === feedback.status}>
                Update Status
              </Button>
            </div>
          </div>
